...
int32_t i2s_app_a2d_data_cb(uint8_t *data, int32_t len)
{
    int test_bits = 32;
    size_t i2s_bytes_read = 0;
    //setup_triangle_sine_waves(test_bits);
    int *samples_data = malloc(len);
    i2s_set_clk(I2S_NUM, SAMPLE_RATE, test_bits, 2);
    i2s_read(I2S_NUM, samples_data, len, &i2s_bytes_read, 100);
    memcpy(data, samples_data, len);
    free(samples_data);


  printf("\r\nTest bits=%d read data=%d len is %d\n", test_bits, i2s_bytes_read, len);
