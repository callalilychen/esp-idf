...
void app_main(void)
{
  ...
  while(true){
    int len = ((test_bits+8)/16)*SAMPLE_PER_CYCLE*4;
  char *samples_data = malloc(len);
  memset(samples_data, 0, len);
//for(int i =0; i < len; i++){
//  printf("%d ", samples_data[i]);
//  }
//  printf("\n=====\n");
  i2s_set_clk(I2S_NUM, SAMPLE_RATE, test_bits, 2);
  i2s_read(I2S_NUM, samples_data, len, &i2s_bytes_read, 100);
  //memcpy(data, sample_data, len)
  for(int i =0; i < i2s_bytes_read; i++){
  printf("%d ", samples_data[i]);
  }
  printf("\nRead bytes %d len is %d\n", i2s_bytes_read, len);
    free(samples_data);
  }
